...
def preload_clean_description(doc):
    mapping = dict()
    #one entry in each line
    for line in doc.split('\n'):
        tokens = line.split()
        ...
        if image_id not in mapping:
            mapping[image_id] = list()
        image_desc = Loader.generate_term(image_desc)
        image_desc = ' '.join(image_desc)
        mapping[image_id].append(image_desc)
    return mapping
...
# tokenFile = 'H:\\1UNIVERSITY\\2020_1\\Truy vấn thông tin đa pt\\project\\Dataset\\Flickr8k_text\\Flickr8k.token.txt'
# # load descriptions
# doc = load_text(tokenFile)
# # parse descriptions
# descriptions = preload_clean_description(doc)
...
# # save to file
# descrOut = 'H:\\1UNIVERSITY\\2020_1\\Truy vấn thông tin đa pt\\project\\Dataset\\Image Caption Generator\\descriptions_clean.txt'
